      ...
      <div className="container mx-auto px-3 sm:px-4 py-3 sm:py-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            <Heart className="text-red-300 flex-shrink-0" size={28} fill="currentColor" />
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold truncate">Traduz Saude</h1>
              <p className="text-[10px] sm:text-xs md:text-sm text-blue-100 hidden xs:block truncate">
                {user
                  ? `${user.account_type === 'specialist' ? 'Especialista' : 'Paciente'}: ${user.full_name}`
                  : 'Documentos medicos em linguagem acessivel'}
              </p>
            </div>
          </div>
